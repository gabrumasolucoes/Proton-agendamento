 ...
 * @param {string} doctorId - Opcional: se fornecido, filtra blocos específicos daquele profissional
 */
async function getBlocksForUser(supabase, userId, doctorId = null) {
    ...
    try {
        ...
        // Se doctorId for fornecido, buscar blocos para:
        // 1. Clínica inteira (doctor_id = null)
        // 2. Este profissional específico (doctor_id = doctorId)
        if (doctorId) {
            query = query.or(`doctor_id.is.null,doctor_id.eq.${doctorId}`);
        } else {
            // Se não tiver doctorId, buscar apenas blocos de clínica inteira
