...
import { getNoShowAnalytics } from '../services/no-show-analytics';
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <h3 className="text-3xl font-bold text-amber-600">{noShowAnalytics.totalCancellations}</h3>
                    </div>
                    <div className="p-3 bg-amber-50 rounded-lg text-amber-600">
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <p className="text-sm font-medium text-gray-500 mb-1">Reincidentes</p>
                      <h3 className="text-3xl font-bold text-gray-800">{noShowAnalytics.frequentOffenders.length}</h3>
                    </div>
                    <div className="p-3 bg-gray-100 rounded-lg text-gray-600">
                      <Users className="w-6 h-6" />
                    </div>
                  </div>
                  <p className="text-xs text-gray-500">Pacientes com 2+ ocorrências</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium text-gray-700">Pelo Cliente</span>
                        <span className="text-gray-500">{noShowAnalytics.cancelledByPatient}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                          <div 
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium text-gray-700">Pelo Operador</span>
                        <span className="text-gray-500">{noShowAnalytics.cancelledByOperator}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                          <div 
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium text-gray-700">Falta (Sistema)</span>
                        <span className="text-gray-500">{noShowAnalytics.totalNoShows}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                          <div 
                            className="h-full bg-rose-500 rounded-full transition-all duration-500"
                            style={{ width: `${(noShowAnalytics.totalNoShows / noShowAnalytics.totalCancelled) * 100}%` }}
                          />
                        </div>
                        <span className="text-xs font-semibold text-rose-600 w-12 text-right">
                          {((noShowAnalytics.totalNoShows / noShowAnalytics.totalCancelled) * 100).toFixed(0)}%
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {/* Por Dia da Semana */}
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                  <h3 className="text-lg font-bold text-gray-800 mb-6">Distribuição por Dia da Semana</h3>
                  <div className="space-y-3">
                    {Object.entries(noShowAnalytics.byDayOfWeek).map(([day, stats]) => {
                      const total = stats.noShows + stats.cancellations;
                      return (
                        <div key={day} className="space-y-1">
                          <div className="flex items-center justify-between text-sm">
                            <span className="font-medium text-gray-700">{day}</span>
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden flex">
                              <div 
                                ...
                                style={{ width: `${(stats.noShows / total) * 100}%` }}
                                title={`${stats.noShows} faltas`}
                              />
                              <div 
                                className="h-full bg-amber-400 transition-all duration-500"
                                style={{ width: `${(stats.cancellations / total) * 100}%` }}
                                title={`${stats.cancellations} cancelamentos`}
                              />
                            </div>
                            <span className="text-xs text-gray-500 w-16 text-right">
                              {stats.noShows}F + {stats.cancellations}C
                            </span>
                          </div>
                        </div>
                      );
              ...
              {noShowAnalytics.frequentOffenders.length > 0 && (
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                  <h3 className="text-lg font-bold text-gray-800 mb-6">Pacientes com Múltiplas Ocorrências</h3>
                  <div className="space-y-2">
                    {noShowAnalytics.frequentOffenders.map((offender, index) => (
                      <div key={offender.patientId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                        <div className="flex items-center gap-3">
                          <span className="text-xs font-bold text-gray-400 w-6">#{index + 1}</span>
                          <div>
                            <p className="font-medium text-gray-800">{offender.patientName}</p>
                            <p className="text-xs text-gray-500">ID: {offender.patientId.slice(0, 8)}...</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="px-3 py-1 bg-rose-100 text-rose-700 rounded-full text-sm font-semibold">
                            {offender.occurrences} ocorrências
                          </span>
                  ...
                  <p className="text-xs text-gray-500 mt-4">
                    💡 Dica: Considere entrar em contato com estes pacientes para entender os motivos das ocorrências
                  </p>
                </div>
              )}
            </>
